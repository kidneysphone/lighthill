...
def savedaily60_quotes(code,thread):
    ...
    if(serverNo==2):
        ...
        sh = ts.get_hist_data('sh',ktype='60',start=date);
        sz = ts.get_hist_data('sz',ktype='60',start=date);
        hs300 = ts.get_hist_data('hs300',ktype='60',start=date);
        sz50 = ts.get_hist_data('sz50',ktype='60',start=date);
        zxb = ts.get_hist_data('zxb',ktype='60',start=date);
        cyb = ts.get_hist_data('cyb',ktype='60',start=date);
    ...
    for c in code:
        ...
        try:
            ...
            tt = ts.get_hist_data(c,ktype='60',start=d3);
            ...
            print Exception, e;
            # daily[c].delete_many({'date':{'$gt':date}});
            #tt = ts.get_hist_data(c,ktype='60',start=date);
            #if(not tt.empty):
            #    last = daily[c].find({}).sort([("date",mongo.DESCENDING)]).limit(1);
            #    last = list(last)[0];
            ...
            #print c;
                        # stra1 = threading.Thread(target=update,args=(c,json.loads(tt.to_json(orient='records')),collection,));
                        # stra1.start();
    t1 = threading.Timer(timer_interval,savedaily60_quotes,(code,thread,));
    t1.start();
...
while(needsolve>0):
    ...
    if(j[date1]=='0'):
        ...
        count += 1;
        needsolve -= step;
        t=threading.Timer(0,savedaily60_quotes,(code,count,));
        t.start();
    else:
        exit();
