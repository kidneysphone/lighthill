...
def savedaily15_quotes(code,thread):
    ...
    if(serverNo==2):
        ...
        sh = ts.get_hist_data('sh',ktype='15',start=date);
        sz = ts.get_hist_data('sz',ktype='15',start=date);
        hs300 = ts.get_hist_data('hs300',ktype='15',start=date);
        sz50 = ts.get_hist_data('sz50',ktype='15',start=date);
        zxb = ts.get_hist_data('zxb',ktype='15',start=date);
        cyb = ts.get_hist_data('cyb',ktype='15',start=date);
    ...
    for c in code:
        ...
        try:
            ...
            tt = ts.get_hist_data(c,ktype='15',start=d3);
            if(not tt.empty):
                ...
                print c;
                        # stra1.start();
        except Exception,e:                        # stra1 = threading.Thread(target=update,args=(tt,collection,));

            print Exception,e;
            # daily[c].delete_many({'date':{'$gt':date}});
            #tt = ts.get_hist_data(c,ktype='15',start=date);
            #if(not tt.empty):
            #    tt['date'] = tt['open'].keys();
            #    last = daily[c].find({}).sort([("date",mongo.DESCENDING)]).limit(1);
            #    last = list(last)[0];
            ...
            #            tt['amount'] = list(real)[0]['amount'];
            #            daily[c].insert(json.loads(tt.to_json(orient='records')));
                        # stra1 = threading.Thread(target=update,args=(tt,collection,));
                        # stra1.start();
    t1 = threading.Timer(timer_interval,savedaily15_quotes,(code,thread,));
    t1.start();
...
while(needsolve>0):
    ...
    if(j[date1]=='0'):
        ...
        count += 1;
        needsolve -= step;
        t=threading.Timer(0,savedaily15_quotes,(code,count,));
        t.start();
    else:
        exit();
