...
def getallcode():
    ...
    try:
        ...
        # print codea,'code':{'$gte':'002222','$lt':'002622'}
        for item in codea:
...
# # if(serverNo==2):
#sh = ts.get_hist_data('sh',ktype='15',start=start,end=end);
#sz = ts.get_hist_data('sz',ktype='15',start=start,end=end);
#hs300 = ts.get_hist_data('hs300',ktype='15',start=start,end=end);
#sz50 = ts.get_hist_data('sz50',ktype='15',start=start,end=end);
#zxb = ts.get_hist_data('zxb',ktype='15',start=start,end=end);
#cyb = ts.get_hist_data('cyb',ktype='15',start=start,end=end);
...
#daily15['sh'].insert(json.loads(sh.to_json(orient='records')));
#daily15['sz'].insert(json.loads(sz.to_json(orient='records')));
#daily15['hs300'].insert(json.loads(hs300.to_json(orient='records')));
#daily15['sz50'].insert(json.loads(sz50.to_json(orient='records')));
#daily15['zxb'].insert(json.loads(zxb.to_json(orient='records')));
#daily15['cyb'].insert(json.loads(cyb.to_json(orient='records')));
#for code in codelist:
#     print code;
#     tt = ts.get_hist_data(code,ktype='15',start=start,end=end);
#     if(not tt.empty):
#         tt['date'] = tt['open'].keys();
#         daily15[code].insert(json.loads(tt.to_json(orient='records')));
#

# step2 update original data
for code in codelist:
    collection = daily15[code];
